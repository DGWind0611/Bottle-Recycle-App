...
public class FirebaseSync {
    ...
    private void processRecycleRecord(DataSnapshot snapshot) {
        try {
            ...
            // 創建 RecycleRecord 並更新 SQLite
            RecycleRecord record = new RecycleRecord(recordId, userName, userTag, recycleStationId, recycleDate, recycleWeight, earnMoney);
            boolean isUpdated = dbHelper.addRecycleRecord(record);

            if (isUpdated) {
                Log.d("FirebaseSync", "New record added/updated in SQLite: " + recordId);
                showSuccessDialog("成功同步回收紀錄！");
            }
        } catch (Exception e) {
